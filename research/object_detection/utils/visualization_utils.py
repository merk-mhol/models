...
def my_visualize_boxes_and_labels_on_image_array(
  image,
  boxes,
  classes,
  scores,
  ...
  # Create a display string (and color) for every box location, group any boxes
  # that correspond to the same location.
  my_box_to_label_map = collections.defaultdict(list)
  box_to_display_str_map = collections.defaultdict(list)
  box_to_color_map = collections.defaultdict(str)
  box_to_instance_masks_map = {}
  box_to_instance_boundaries_map = {}
    ...
    if scores is None or scores[i] > min_score_thresh:
      ...
      if scores is None:
        ...
        if classes[i] in category_index.keys():
          ...
          class_name = 'N/A'
        my_box_to_label_map[box].append(class_name)
        if not skip_labels:
          if not agnostic_mode:
            display_str = str(class_name)
        if not skip_scores:
  ...
  # Zuordnung von Farbe zu Klasse (für Legende)
  my_color_to_label_map = {}
  ...
  for box, color in box_to_color_map.items():
    ...
    # Klassen-Farben-Dict ergänzen
    my_color_to_label_map[my_box_to_label_map[box][0]] = color

  result = [image, my_color_to_label_map]
